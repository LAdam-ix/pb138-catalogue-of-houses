...
export const checkAccount = async (
  ...
  if (data.email) {
    const accountMail = await tx.account.findUnique({
      where: { email: data.email },
    });
    if (accountMail != null) {
      return Result.err(new DuplicateRecordError('The specified email is allready registered!'));
    }
  }
...
export const checkOrder = async (
  ...
  if ((data.designerId && data.designerId !== order.designerId)
    || (data.customerId && data.customerId !== order.customerId)) {
    return Result.err(new WrongOwnershipError('Forbiden: You have no access to this data!'));
  }
